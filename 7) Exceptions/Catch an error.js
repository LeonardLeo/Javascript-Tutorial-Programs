...
The code above will try a statement. If any errors are thrown, the logic will flow into the catch block. 
The line console.log(ex); 
will only execute if an error (ex) is thrown by readFile.
The EISDIR is thrown in Node.JS when the target is a directory when we were expecting it to be a file. 
So this is a very realistic example! Here are a list of common system errors.
